...
function loadUserPanel() {
  ...
  greetingTitleNode.textContent = `Hello, ${appState.currentUser.login}`;
...
function loadMainPage() {
  ...
  const [userListSelect, taskReadyListSelect, taskInProgressListSelect] =
    updateAllListSelect();

  if (!appState.currentUser.hasAdmin) {
    addBtnList.forEach((item) => {
      if (item.dataset.state == "ready")
  ...
  formList.forEach((item, index) => {
    ...
    item.addEventListener("submit", (e) => {
      ...
      if (state == "ready") {
        ...
        const user_id =
          userListSelect.options[userListSelect.selectedIndex].value;
      ...
      if (state == "in-progress" && taskReadyListSelect.options.length > 0) {
        const currentOption =
          taskReadyListSelect.options[taskReadyListSelect.selectedIndex];
      ...
      if (state == "finished" && taskInProgressListSelect.options.length > 0) {
        const currentOption =
          taskInProgressListSelect.options[
            taskInProgressListSelect.selectedIndex
          ];
...
function loadAddUserPage() {
  ...
  users.reverse().forEach((item, index) => {
    ...
    const taskCounterReady = document.querySelectorAll(
      ".task-counter-list__item-ready"
    )[index];
